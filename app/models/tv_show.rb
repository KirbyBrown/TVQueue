class TvShow < ApplicationRecord
  ...
  def add_or_update
    show = self
    if show.updated_at.nil? || show.updated_at < (Time.now - 1.minute)
  ...
  def add_or_update_episodes
    show = self
    if show.episodes.last.nil? || show.episodes.last.updated_at < (Time.now - 1.minute)
  ...
  def add_or_update_network
    ...
    unless show.network.nil?
      ...
      if network.updated_at.nil? || network.updated_at == network.created_at || network.updated_at < (Time.now - 1.minute)
